...
$_SESSION['ratesType'] = isset($_POST['ratesType']) ? $_POST['ratesType']: 'Day';
...
$departure =  $_SESSION['departure']. ' ' . $_SESSION['end_time'];
    ...
    if (isset($_SESSION['person'])) {
        ...
        $pax =  $_SESSION['person'];
         $query = "SELECT * FROM `tblroom`  WHERE AccomodationType='{$type}' AND  `NUMPERSON` >=$pax";
    }else{

        $query = "SELECT * FROM `tblroom` WHERE AccomodationType='{$type}' ";
    }
...
if($days==1){
       ...
       switch($_SESSION['ratesType']){
           case 'Day' :
        $msg = 'Available '.$type.' for ' .$days .'day in the  ' . $_SESSION['ratesType']. ' Tour (09:00am-04:00pm)';
               break;
            case 'Night' : 
        $msg = 'Available '.$type.' for ' .$days .'day in the ' . $_SESSION['ratesType']. ' Tour (03:00pm-10:00pm)';
                break;
            default :
        $msg = 'Available '.$type.' for ' .$days .'day in the  ' . $_SESSION['ratesType']. ' Tour (09:00am-04:00pm)';
     ...
     switch($_SESSION['ratesType']){
           case 'Day' :
        $msg = 'Available '.$type.' for ' .$days .'day(s) in the  ' . $_SESSION['ratesType']. ' Tour (09:00am-04:00pm)';
               break;
            case 'Night' : 
        $msg = 'Available '.$type.' for ' .$days .'day(s) in the ' . $_SESSION['ratesType']. ' Tour (03:00pm-10:00pm)';
                break;
            default :
        $msg = 'Available '.$type.' for ' .$days .'day(s) in the  ' . $_SESSION['ratesType']. ' Tour (09:00am-04:00pm)';
